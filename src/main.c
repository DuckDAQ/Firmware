...
int main (void)
{
	...
	while(1)
	{
		CMD_t incomingCMD; //struct for incoming command
		if(USB->available())
		{
			/*
	...
	//{
//
		//if(udi_cdc_get_nb_received_data()) //Cekira ce je kaksen nov ukaz
		//{
			////?vrjetn bi mogu poslt udi_cdc_getc() parserju
			//if(udi_cdc_getc() == 's') //Parsanje ukaza
			//{
				//core_configure(&master_settings); //nastavitev cora z daq_settingsi, ki mu jih passamo
				//core_start(); //start core
				//while(core_status_get() == CORE_RUNNING) //Cakamo v while loopu dokler se ne izvede ADC
				//{
					//if(core_new_data_ready()) //Ko je ADC finisiran, povprecimo podatke in jih poslemo nazaj prek USB
					//{
